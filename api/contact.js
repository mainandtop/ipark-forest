/* =======================================================================
   ...
   필요한 환경변수 (Vercel > Settings > Environment Variables):
     ...
     RECEIVER_EMAILS  : 수신 이메일(콤마로 여러 개 지정 가능 → 여기만 늘리면 됨)
                        예) "momoseya55@gmail.com, sales@dongmun.co.kr"
...
module.exports = async function handler(req, res) {
  ...
  const { from_name, phone, visit_date, visit_time } = body || {};
  ...
  const siteName = process.env.SITE_NAME || '운정 아이파크 포레스트';
    ...
    <div style="font-family:'Apple SD Gothic Neo',sans-serif;max-width:560px;margin:0 auto;border:1px solid #eee;border-radius:10px;overflow:hidden">
      ...
      <div style="padding:26px 28px;background:#fff">
        <table style="width:100%;border-collapse:collapse;font-size:14px">
          ...
          <tr>
            <td style="padding:12px 0;color:#1B2A4A;font-weight:700">방문예약 일시</td>
            <td style="padding:12px 0">${esc(visit_date || '미선택')}${visit_time && visit_time !== '미선택' ? ' ' + esc(visit_time) : ''}</td>
          </tr>
        </table>
      </div>
